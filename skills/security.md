...
- NoSQL operator injection — when passing user input into a DB such as MongoDB query object, validate that string fields are actually strings (`z.string()` blocks `{ $gt: '' }` and `{ $where: '...' }` objects from being passed as field values)
...
API keys, tokens, connection strings, and private keys must come from environment variables. Never commit them to source control.
...
2. Enforce a maximum file size **before** reading the full stream — reject before buffering
