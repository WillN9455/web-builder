...
**See also:** [`CLAUDE.md`](./CLAUDE.md) Framework Flow Map (§"Framework Flow Map — Cross-Reference Index") for complete file dependency table; [`workflows/README.md`](./workflows/README.md) §"Workflow Execution Notes" for orchestration patterns.
...
- **Trigger**: User's idea/pain point from `idea.txt` → see CLAUDE.md Framework Flow Map row "PRD/templates/prd-template.md" | Upstream input: [`idea.txt`](./idea.txt)
- **Responsibilities**:
...
- **Output**: `PRD/<project-name>/prd.md` → consumes: [`idea.txt`](./idea.txt), user answers | produces: design triggers, QA criteria | see also CLAUDE.md Flow Map row "PRD/<project>/prd.md"

### Requirements Reviewer (1 agent)
- **Trigger**: New PRD from BA Agent at `PRD/<project-name>/prd.md` → see CLAUDE.md Flow Map row "PRD/<project>/prd.md" | Upstream input: PRD template sections (§8 user stories, §6 UX principles, §9 supporting docs)
- **Responsibilities**:
...
- **Trigger**: Approved PRD from Main Orchestrator → see CLAUDE.md Flow Map row "PRD/<project>/prd.md" | Upstream inputs: PRD §6 UX Design Principles (→ typography/spacing scale), PRD §8 User Stories (→ component list via `design-system/components/README.md` index)
- **Responsibilities**:
...
- **Output**: [`design-system/<project-name>/`](./design-system/) with tokens (compiled CSS + spec files), components (per component specs), and states — feeds downstream: Code Agents, Playwright tests | see also CLAUDE.md Flow Map rows for each token/component/state file
...
- **Output**: Feature branches with implemented code + unit tests → feeds downstream: Dev Reviewers, Playwright tests | see CLAUDE.md Flow Map rows for `code-builder/config-rules.md`, each skill file, and testing README
...
- **Trigger**: Code pushed by a Code Agent (task in `review` state) → see CLAUDE.md Flow Map row "skills/coding-guidelines.md" + all skill files | Upstream inputs: feature branch code, design spec (`design-system/components/<feature>.md`), state specs (`design-system/states/`), PRD acceptance criteria (§8 User Stories)
- **Responsibilities**:
...
- **Output**: Test results + any bug tickets → feeds back to Code Agents; test suite becomes permanent artifact in `testing/playwright/features/<feature>/` | see CLAUDE.md Flow Map row "testing/playwright/README.md"
...
2. **Artifact files** — PRDs, design specs, code diffs are written as files others read. See CLAUDE.md Framework Flow Map (§"Framework Flow Map — Cross-Reference Index") for complete artifact chain: every file's outputs column shows which downstream files consume it.
...
Follows `workflows/README.md` Workflow sequences. Each step references the CLAUDE.md Framework Flow Map row for artifact dependencies.

| Step | Action | Upstream input (from Flow Map) | Orchestrates (next agent) |
|------|--------|-------------------------------|--------------------------|
...
| 6 | → Design Agents create designs in parallel, peer-review each other | `design-system/tokens/README.md` + all token/component/state files from CLAUDE.md Flow Map | Main Orchestrator (when approved) |
...
| 11 | → Done | All artifacts documented per CLAUDE.md Framework Flow Map "Outputs" column for each file touched | — |
